...
def handle_verdict():
    while True:
        ...
        if verdict['verdict'] == 1: # assume 1 is malicious, 0 is clean
            kill_pid(verdict['pid'])
...
def driver(json_sample):
    ...
    if is_match_in_blacklist(api_seq_hash):
        print "[+] Found matching sequence for process[%s] in the blacklist" % str(pid)
        # Add verdict to the queue: kill or do nothing
        verdict_queue.put({"pid":pid, "verdict":1})
        return True
        ...
        # ML
        ml_verdict = backup_verdict(pid)
        if ml_verdict:
            verdict_queue.put({"pid":pid, "verdict":1})
        else:
            verdict_queue.put({"pid":pid, "verdict":0})
        return ml_verdict
...
def receive_api_logs_from_sycall_plugin():
    while True:
        ...
        procName, apiName = log.split(",")[2], log.split(",")[5]
        # print("Process:", procName)
        # print("API:", apiName)

        # Check if process was already being tracked
        if procName not in uniqApis:
            print "[+] New Process Found [%s], Monitoring Now..." % str(procName)
        ...
        # processQueues[procName].append(apiName)
            ...
            if processQueues[procName].qsize() == API_SEQUENCE_SIZE: # set to 1000
...
def main():
    ...
    print "[+] Initiating new Thread for handling verdict..."
    verdict_handling_thread = threading.Thread(target=handle_verdict)
    worker_threads.append(verdict_handling_thread)
    verdict_handling_thread.start()

    print "[+] Initiating new Thread for process blacklist..."
    blacklist_handling_thread = threading.Thread(target=black_list)
    worker_threads.append(black_list)
    blacklist_handling_thread.start()
